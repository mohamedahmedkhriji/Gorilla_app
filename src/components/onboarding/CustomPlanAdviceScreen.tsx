...
const COPY = {
  ...
  it: {
    title: 'Consigli AI per il tuo piano personalizzato',
    subtitle: 'Il tuo piano e salvato. Ecco alcuni miglioramenti su misura per il tuo profilo e i tuoi orari.',
    loading: 'Stiamo rivedendo il tuo piano e generando consigli...',
    error: 'Impossibile generare consigli per il piano personalizzato.',
    summaryFallback: 'Il tuo piano personalizzato sembra solido. Continua con progressione costante e buon recupero.',
    strengths: 'Cosa sta funzionando',
    recommendations: 'Consigli AI',
    cta: 'Vai alla Home',
  },
  de: {
    title: 'KI-Tipps fuer deinen individuellen Plan',
    subtitle: 'Dein Plan wurde gespeichert. Hier sind Verbesserungen fuer dein Profil und deinen Zeitplan.',
    loading: 'Dein Plan wird geprueft und Tipps werden erstellt...',
    error: 'Individuelle Plan-Tipps konnten nicht erstellt werden.',
    summaryFallback: 'Dein individueller Plan sieht gut aus. Bleib bei konstanter Progression und guter Erholung.',
...
export function CustomPlanAdviceScreen({
  ...
  }, [copy.error, isCustomPlanReady, language, onboardingData, resolvedUserId]);
      ...
      {!loading && !error && (
        <>
          <div className="rounded-2xl border border-accent/30 bg-accent/10 p-4">
            <p className="text-sm text-white">{advice?.summary || copy.summaryFallback}</p>
          </div>

          {Array.isArray(advice?.strengths) && advice.strengths.length > 0 && (
            <div className="rounded-2xl border border-white/10 bg-card/70 p-4">
              <p className="text-xs font-semibold uppercase tracking-[0.1em] text-accent">{copy.strengths}</p>
          ...
          {Array.isArray(advice?.recommendations) && advice.recommendations.length > 0 && (
            <div className="rounded-2xl border border-white/10 bg-card/70 p-4">
              <p className="text-xs font-semibold uppercase tracking-[0.1em] text-accent">{copy.recommendations}</p>
              <div className="mt-2 space-y-1.5 text-sm text-text-secondary">
