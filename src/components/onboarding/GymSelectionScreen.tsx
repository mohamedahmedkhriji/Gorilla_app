  ...
  return (
    <div className="flex-1 flex flex-col space-y-6">
      <div className="space-y-2">
        <h2 className="text-2xl font-light text-white">{isArabic ? 'اختر ناديك' : 'Select Your Gym'}</h2>
        <p className="text-text-secondary">
          {isArabic ? 'اعثر على موقع تدريبك الأساسي.' : 'Find your primary training location.'}
        </p>
      </div>

      <div className="relative">
        <Input placeholder={isArabic ? 'ابحث عن النوادي القريبة...' : 'Search gyms nearby...'} className="pl-10" />
        <Search
      ...
      <div className="space-y-3">
        <div className="p-4 bg-card rounded-xl border border-accent/20 flex items-center gap-4 cursor-pointer hover:bg-white/5 transition-colors">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-text-secondary">{isArabic ? 'يبعد 0.8 ميل' : '0.8 miles away'}</p>
          </div>
        ...
        <div className="p-4 bg-card rounded-xl border border-white/5 flex items-center gap-4 cursor-pointer hover:bg-white/5 transition-colors">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-text-secondary">{isArabic ? 'يبعد 1.2 ميل' : '1.2 miles away'}</p>
          </div>
        </div>
      </div>
      ...
      <div className="p-4 bg-yellow-500/10 border border-yellow-500/20 rounded-xl flex gap-3">
        ...
        <p className="text-xs text-yellow-200/80 leading-relaxed">
          {isArabic
            ? 'هذا التطبيق مخصص للتدريب في النادي فقط. تمارين المنزل غير مدعومة حاليًا.'
            : 'This app is designed for gym training only. Home workouts are not currently supported.'}
        </p>
      </div>

      <div className="flex-1" />

      <Button onClick={onNext}>{isArabic ? 'تأكيد النادي' : 'Confirm Gym'}</Button>
    </div>);
