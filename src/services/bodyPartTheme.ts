...
export const getBodyPartImage = (muscleGroup: unknown) => {
  ...
  if (
    ...
    || key === 'arms'
    || key.includes('البايسبس')
  ) return bicepsImage;
  if (key.includes('calf') || key.includes('السمانة')) return calvesImage;
  if (key.includes('chest') || key.includes('pect') || key.includes('الصدر')) return chestImage;
  if (
  ...
  if (key.includes('glute') || key.includes('الألوية')) return glutesImage;
  if (key.includes('hamstring') || key.includes('الخلفية')) return hamstringsImage;
  if (key.includes('adductor') || key.includes('المقربات')) return adductorsImage;
  if (
    key.includes('quad')
    || key.includes('thigh')
    || key === 'legs'
    || key.includes('leg')
    || key.includes('الرباعية')
    || key.includes('الأرجل')
  ...
  if (key.includes('الأكتاف')) {
    if (key.includes('الخلفية')) return shoulderBackImage;
    return shouldersImage;
  }
  if (key.includes('tricep') || key.includes('الترايسبس')) return tricepsImage;
