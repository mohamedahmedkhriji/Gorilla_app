...
const MIN_LOGIN_LOADING_MS = 2200;
...
export const LoginPage: React.FC<LoginPageProps> = ({ onLoginSuccess }) => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await wait(Math.max(0, MIN_LOGIN_LOADING_MS - elapsedLoadingMs));
        ...
        <section className="login-cyber-card px-5 py-6 md:px-7 md:py-8">
          ...
          {!isStandalone && deferredPrompt ? (
            <button
              ...
              className="login-cyber-download mb-4 flex w-full items-center justify-center gap-2 px-4 py-3 text-sm font-semibold transition-colors"
            >
              <Download size={16} />
              {copy.download}
            </button>
          ...
          {error && <div className="login-cyber-error mb-4 px-3 py-2 text-sm">{error}</div>}

          <form onSubmit={handleLogin} className="space-y-4">
            <div>
              <label className="login-cyber-label block mb-2">{copy.email}</label>
              <div className="relative">
                ...
                <input
                  ...
                  className="login-cyber-input w-full pl-10 pr-3 py-3"
                />
              </div>
            </div>

            <div>
              <label className="login-cyber-label block mb-2">{copy.password}</label>
              <div className="relative">
                ...
                <input
                  ...
                  className="login-cyber-input w-full pl-10 pr-10 py-3"
                />
                <button
                  type="button"
                  onClick={() => setShowPassword(!showPassword)}
                  className="login-cyber-eye absolute right-3 top-1/2 -translate-y-1/2"
            ...
            <button
              type="submit"
              disabled={loading}
              className="login-cyber-submit w-full disabled:opacity-60 disabled:cursor-not-allowed"
            >
              {loading ? copy.loggingIn : copy.login}
            </button>
          </form>

          <div className="login-cyber-note mt-5 text-center text-sm">
            {copy.needAccount}
            <span className="text-accent font-semibold"> {copy.contactCoach}</span>
          </div>

          <div className="login-cyber-admin text-center mt-4 pt-4">
            <a href="/admin.html" className="text-xs transition-colors">
              {copy.adminLogin}
            </a>
          </div>
        </section>
