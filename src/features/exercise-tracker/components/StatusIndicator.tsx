...
const TONE_STYLES: Record<IndicatorTone, string> = {
  ...
  warning: 'border-amber-300/55 bg-amber-400/16 text-amber-50 shadow-[0_18px_55px_rgba(251,191,36,0.12)]',
  good: 'border-emerald-400/35 bg-emerald-500/14 text-emerald-50',
  bad: 'border-rose-300/55 bg-rose-500/16 text-rose-50 shadow-[0_18px_55px_rgba(251,113,133,0.14)]',
};
...
const TONE_ICONS = {
  ...
  good: CheckCircle2,
  bad: XCircle,
} satisfies Record<IndicatorTone, typeof Info>;
    ...
    <div className={`rounded-[28px] border p-5 transition-colors duration-200 ${TONE_STYLES[tone]}`}>
      <div className="flex items-center gap-4">
        <div className="relative flex h-14 w-14 items-center justify-center rounded-full border border-current/20 bg-black/10">
          ...
          <Icon size={18} className="absolute text-black/75" />
        </div>
