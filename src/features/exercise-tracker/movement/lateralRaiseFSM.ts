...
const getAverageRawWristHeightRatio = (frame: ProcessedFrame) => average(
  frame.rawFeatures.wristHeightRatioLeft,
  frame.rawFeatures.wristHeightRatioRight,
);
...
const evaluateRawPhase = (
  ...
  const thresholds = MOVEMENT_THRESHOLDS.lateralRaise;
  const primarySignal = getAverageWristHeightRatio(frame);
  const rawPrimarySignal = getAverageRawWristHeightRatio(frame);
  ...
  const validSetup = (
    ...
    && movementMagnitude <= thresholds.setupVelocityAbsMax
    && asymmetry <= thresholds.readySymmetryMax
    && torsoLean <= thresholds.readyTorsoLeanMax
  );
  const nearPeak = greaterThanOrEqualWithHysteresis(
    primarySignal,
  ...
  return {
    ...
    nextPositiveFrames,
    nextNegativeFrames,
    leftSignal: frame.features.wristHeightRatioLeft,
    rightSignal: frame.features.wristHeightRatioRight,
    asymmetry,
    torsoLean,
    validSetup,
    nearPeak,
...
export const createLateralRaiseState = () => createInitialMovementState(EXERCISE);

export const updateLateralRaiseFSM = (
  frame: ProcessedFrame,
...
export const getLateralRaiseOutput = (state: MovementState) => buildMovementOutput(state);
